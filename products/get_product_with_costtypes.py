...
def generate_pipeline():
    pipeline = [
        ...
        {
            "$group": {
                "_id": {"_id": "$_id", "code": "$code", "name": "$name", "index": "$index", "tags": "$tags", "parent": "$parent"},
                "costTypes": {
                    "$addToSet": {
...
def main():
    ...
    ws.cell(row=1, column=15).value = 'costTypeTags'
    n = 1
    for i, doc in enumerate(results, 1):
        ...
        for ct in cts:
            ...
            if tags:
                tags = ['{}:{}'.format(tag.get('orderTypeCode', ''), tag.get('isVisible', '')) for tag in tags]
