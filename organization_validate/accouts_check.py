...
if __name__ == '__main__':
    ...
    for i in range(2, ws.max_row + 1):
        ...
        if account is None:
            ws.cell(row=i, column=6).value = 'invalid'
        else:
            account_id = account['_id']
            ws.cell(row=i, column=7).value = str(account_id)
            ws.cell(row=i, column=8).value = account['types'][0]
            username_doc = epmongo.get_doc_by_query('users', {'account': account_id, 'isSuperAdmin': True})
            if username_doc is None:
                ws.cell(row=i, column=6).value = 'redundant'
            else:
                username = username_doc['username']
                ws.cell(row=i, column=6).value = username
    wb.save(file_name)
