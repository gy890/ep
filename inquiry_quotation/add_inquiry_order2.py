...
def consigner(consigner_cookie):
    def decorate(func):
        @wraps(func)
        def wrapped(*args, **kwargs):
            result = func(consigner_cookie, *args, **kwargs)
            return result
...
def consignee(consignee_cookie):
    def decorate(func):
        @wraps(func)
        def wrapped(*args, **kwargs):
            result = func(consignee_cookie, *args, **kwargs)
            return result
...
@timeit.timeit
@consigner(consigner_cookie)
def add_inquiry_order(cookie, data):
    headers = HEADERS.copy()
    if cookie:
        headers.setdefault('x-session-id', cookie)
    payload = {
...
@timeit.timeit
def get_inquiry_order(order_id):
    request_data = {'orderId': order_id}
    consignee_id = USERS.ACCIDS[1]['_id']
    for inquiry_order in _find_inquiry_orders_by_order_id(request_data).json()['response']:
...
def main():
    ...
    res = add_inquiry_order(inquiry_order_params).json()
    ...
    for k, v in products.items():
        ...
        add_order_entry_and_config_to_order(data).json()

    add_real_inquiry_order_params = generate_add_real_inquiry_order_params(order_type, order_id)
    add_real_inquiry_order(add_real_inquiry_order_params)
    ...
    for i, entry in enumerate(res['response']['orderEntries'], 1):
        ...
        if product_code != 'PTAGT':
            ...
            res = update_product_config(product_config).json()
            order_v = res['response']['__v']
    ...
    create_inquiry_orders(create_inquiry_orders_params)

    input('Press any key to continue')

    inquiry_order_id = get_inquiry_order(order_id)  # 获取inquiryOrder._id

    logging.info('--> findInquiryOrderById...')
    res = find_inquiry_order_by_id({'id': inquiry_order_id}).json()
    ...
    for i, entry in enumerate(res['response']['orderEntries'], 1):
        ...
        cost_config = generate_product_costs(order_type, order_id, inquiry_order_id, currency, product_id, entry_id,
                                             cost_types)
        update_inquiry_order_cost_items(cost_config)

    input('Press any key to continue')

    logging.info('--> setOrderQuotation...')
    set_order_quotation({"id": inquiry_order_id, "orderId": order_id})
    ...
    appointment_agent(request_data)
